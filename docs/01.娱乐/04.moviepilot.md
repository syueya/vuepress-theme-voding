...
CookieCloud容器部署，主要是设置个端口。

![img](./img/0410.png)

部署完成后点击浏览器输入IP:端口，可以看到 Hello World！字样，表示安装成功。
...
打开浏览器输入IP:端口进入mp界面，输入环境中配置的超级管理员用户名（默认admin）、密码（默认password）进入首页。
